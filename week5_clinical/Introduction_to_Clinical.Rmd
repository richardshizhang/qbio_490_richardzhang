...
```{r}
write.csv(clinic, "/home1/rszhang/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/rszhang/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
```{r Exercise 3.1}
# write code here
str(clinic) # 1079 rows, 112 columns
typeof(clinic$gender) #"character"
colnames(clinic)
clinic[5,"age_at_diagnosis"] # 50
p1_follow_up_age <-(-as.integer(clinic[1,"birth_days_to"]) - as.integer(clinic[1,"last_contact_days_to"]))/365 #age in days - days since last contact converted to years
p1_follow_up_age
```
...
cleaned_clinic[,c("last_contact_days_to","death_days_to")]
```
...
probability of survival per day
...
p is 0.016, which is less than 0.05 and is therefore statistically significant, indicating that the differences in data between age groups are unlikely to be because of chance.
